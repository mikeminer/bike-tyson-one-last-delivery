...
- The continuous body is baked from smooth-union anatomical volumes with `scripts/build-anatomy.mjs` and animated with 13 bones. Wheels and drivetrain are geometry created in source. `public/character/skin-albedo-v2.png` is a reference-conditioned generated skin swatch; its exact prompt and hash are in `public/character/texture-provenance.json`. Rest-space triplanar projection avoids UV seams during movement; skin, rubber and leather use source-generated micro-bump detail. Feet use two-bone inverse kinematics, arms animate for a punch, and wheel/crank transforms follow recorded distance. Fixed character meshes are batched by material.
