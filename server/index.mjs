...
async function body(req) {
  let data = ''; for await (const part of req) { data += part; if (data.length > 8192) throw Error('Richiesta troppo grande.'); } return JSON.parse(data || '{}');
}
...
http.createServer(async (req, res) => {
  ...
  if (url.pathname.startsWith('/api/')) {
    ...
    if (budget.count > 90) { res.setHeader('retry-after', '60'); return send(res, 429, { error: 'Troppe richieste. Attendi un minuto.' }); }
    if (req.headers.origin && req.headers.origin !== origin) return send(res, 403, { error: 'Origine non consentita.' });
    if (req.method === 'POST' && (req.headers.origin !== origin || !req.headers['content-type']?.startsWith('application/json'))) return send(res, 403, { error: 'Richiesta non consentita.' });
    try {
      ...
      if (url.pathname === '/api/access' && req.method === 'GET') {
        const session = auth.session(cookie(req)); if (!session) return send(res, 401, { error: 'Accedi con una firma del wallet per verificare i lock.' });
      ...
      return send(res, 404, { error: 'Endpoint non disponibile.' });
    } catch (error) { return send(res, 400, { error: error.message || 'Richiesta non valida.' }); }
  }
